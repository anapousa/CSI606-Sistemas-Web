...
export function CreateBattleModal({ isOpen, onClose, onSubmit, mcs }: CreateBattleModalProps) {
  const [name, setName] = useState('');
  const [date, setDate] = useState('');
  const [location, setLocation] = useState('');
  const [matchups, setMatchups] = useState<Array<{ mc1: string; mc2: string }>>([
    { mc1: '', mc2: '' },
  ]);
  ...
  const handleMatchupChange = (index: number, field: 'mc1' | 'mc2', value: string) => {
